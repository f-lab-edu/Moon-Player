...
describe('IconButton 기능 테스트', () => {
  ...
  test('버튼에 hover를하면 커진다.', async () => {
    const onClick = jest.fn();
    render(<IconButton color="red" active={false} onClick={onClick} name="plus" size="2x" />);
    const button = screen.getByRole('button');
    expect(button).toHaveStyleRule('transform', 'scale(1.3)', { modifier: ':hover' });
  });
  test('버튼을 누르면 onClick이 실행된다.', async () => {
    render(<IconButton color="white" active={false} onClick={onClick} name="pause" size="1x" />);
    const button = screen.getByRole('button');
...
describe('IconButton 스타일 속성 검증', () => {
  test('버튼 active가 true이면 pointer 감지가 안되고 opacity는 0.5가 된다.', () => {
    const onClick = jest.fn();
    render(<IconButton color="red" active={true} onClick={onClick} name="plus" size="2x" />);
    const button = screen.getByRole('button');
...
describe('버튼 사이즈 검증', () => {
  ...
  test('버튼사이즈는 x', () => {
    const onClick = jest.fn();
    render(<IconButton color="white" active={false} onClick={onClick} name="plus" size="1x" />);
    const buttonSize = screen.getByRole('img', { hidden: true });
    expect(buttonSize).toHaveStyle('font-size: 1em');
  });
  test('버튼사이즈는 2x', () => {
    const onClick = jest.fn();
    render(<IconButton color="white" active={false} onClick={onClick} name="plus" size="2x" />);
    const buttonSize = screen.getByRole('img', { hidden: true });
    expect(buttonSize).toHaveStyle('font-size: 2em');
  });
  test('버튼사이즈는 3x', () => {
    const onClick = jest.fn();
    render(<IconButton color="white" active={false} onClick={onClick} name="plus" size="3x" />);
    const buttonSize = screen.getByRole('img', { hidden: true });
...
describe('올바른 아이콘인지 검증', () => {
  ...
  test('play 버튼인지 확인한다', () => {
    const onClick = jest.fn();
    render(<IconButton color="white" active={false} onClick={onClick} name="play" size="1x" />);
